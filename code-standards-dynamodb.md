...
This document contains coding standards for applications that work with DynamoDB. The document outlines a series of **MUST** and **SHOULD**. **MUST**s** are mandatory and must be followed. **SHOULD** are best practices and should be implemented where reasonable. Examples should be treated as **SHOULD**.
