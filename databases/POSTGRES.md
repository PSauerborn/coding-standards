...
`[PG-005]` **SHOULD**: ID fields should consist of high-entropy values, such as UUIDs. Avoid using incrementing integers as IDs. Use UUIDs with hyphens removed for ID columns unless otherwise instructed.

`[PG-006]` **SHOULD**: Primary ID columns should be internal, and generated by the application itself. If entities modeled in a database refer to external resources, create a link table to map internal IDs to external IDs. For example, when using Auth0, user entities should not be keyed on the Auth0 generated ID. Rather, and internal ID should be generated, and a link that that maps the internal ID to the Auth0 ID should be created.

`[PG-007]` **SHOULD**: Columns should have foreign key constraints to other tables where possible to enforce referential integrity.
